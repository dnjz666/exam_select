...
### ADR-019 补充（2026-09）· "工具注册了 ≠ 功能可用"

**触发**：目标复核时发现 `get_college_level_facts` **只在 `tools.py` 里出现** ——
`parser.py` / `narrator.py` / `prompts.py` 全都没有引用它。

**实测后果**：默认配置是 `LLM_PROVIDER=none`（走**确定性路径**），
而 `parser.detect_intent` 里**没有任何意图**匹配"怎么样 / 什么水平 / 算不算好学校"，
于是这类问题落到 `unknown`，被 `_answer_about_school` 当成**"问分数线"**回答：

```
问：浙江工业大学怎么样
答：浙江工业大学 · 新闻传播学类 近几年的投档情况：2025 年最低分 623，最低位次 28,994…
```

—— 考生问"算不算好学校"，得到的是一串位次数字。**答非所问，且层次功能完全不可达。**

**修法（三层补全）**：

1. `parser._INTENT_RULES` 新增 `college_level` 意图，排在 `history` **之后**
   （"XX大学多少分"仍是 history）、`rank` **之前**；覆盖
   `怎么样|好不好|什么水平|算不算好|实力如何|值得报吗|层次|什么档次…`；
2. `chat_service._answer_about_school_level`：与 `_answer_about_school` 明确分工 ——
   那个答"多少分"（历史位次），这个答"什么层次"（判据 + `level_score` 的来源规则），
   并在 `_deterministic_text` 里**先分流**；
3. `narrator.narrate_college_level`：专家口吻转述判据 + caveats + 来源；
4. `prompts.py` 新增禁止条款 **4b**（LLM 路径）：问院校水平必须先调该工具、
   必须说明"985/211/双一流不是唯一标准"、不得给排名或编就业率。

**顺带修掉两个被测试抓出来的展示层缺陷**：

* 叙述器**硬编码**了归一化分母 `/ 31` —— 那是把领域常量偷偷搬进展示层。
  现改由工具返回 `region_top_college_count_max`，叙述器只负责显示。
  （回归用例 `test_denominator_comes_from_payload_not_hardcoded` 会换一个分母验证它真的跟着变。）
* 省份显示成了内部键 `zhejiang` 而非 `浙江`（叙述器漏用 `province_name()`）。

**验收（真实问答，非"应该通过"）**：

| 提问 | 意图 | 结果 |
|---|---|---|
| 浙江工业大学怎么样 | `college_level` | 判据 + 0.60 + 来源 + "不等于层次低" caveat ✅ |
| 浙江工业大学算不算好学校 | `college_level` | 同上 ✅ |
| 杭州电子科技大学什么水平 | `college_level` | ✅ |
| 浙江工业大学分数线 | `history` | 仍答历史位次，**未被抢走** ✅ |
| 霍格沃茨大学怎么样 | `college_level` | 诚实回绝（库里没有，不编造）✅ |

新增 `tests/test_agent_college_level.py`（12 例，含"输出数字 ⊆ 载荷数字"的防编造断言）。

### 本轮发现的**遗留缺陷**（未修，列入 M7）

* **首次回复的免责声明硬编码"当前数据为模拟数据"**（`chat_service.DISCLAIMER`）——
  对浙江考生是**事实错误**（他们看到的是省考试院真实投档数据）。
  正确做法是按 `is_synthetic` / `source_url` 前缀（`real://` vs `synthetic://`）动态生成。
  本项属独立缺陷，未纳入 ADR-019 范围，避免未经验证的顺手改动。
* **上海 `wen_hit_rate` = 84.30%**（< 85%，ADR-019 之前即存在，已基线对照确认）。
* **推荐列表截断方式**：`limit=60` 时浙工大仍在稳档第 68 位，进不了列表。
  层次与地区都已修好，截断是唯一卡点（HANDOVER §7 第 3 条）。


---

## ADR-020 · 推荐列表的截断方式：从"取前 N"改为"按分层配额取样"（M7 第 3 项）

- **日期**：2026-09
- **状态**：已采纳
- **触发**：M7 清单第 3 项（M6 手测发现），本轮开工复核后确认并修复。

### 缺陷（实测）

`recommend_service.recommend()` 原先是「按 ``(tier, -utility, unit_id)`` 排序后取前 ``limit``」，
而 ``TIER_ORDER`` 把 ``CHONG`` 排在最前。于是当 ``limit`` 小于 CHONG 候选数时，
**返回的整页全是"冲"**。浙江 630 分实测：

| limit | 修复前返回分布 | 修复后 |
|---|---|---|
| 8 | `{'CHONG': 8}` | `{'CHONG': 3, 'WEN': 3, 'BAO': 2}` |
| 20 | `{'CHONG': 20}` | `{'CHONG': 8, 'WEN': 7, 'BAO': 4, 'DIAN': 1}` |
| 60 | `{'CHONG': 60}` | `{'CHONG': 23, 'WEN': 22, 'BAO': 11, 'DIAN': 4}` |
| 100 | `{'CHONG': 92, 'WEN': 8}` | `{'CHONG': 38, 'WEN': 37, 'BAO': 19, 'DIAN': 6}` |

而全池明明有 **BAO 62 / DIAN 7,824**。考生看到"一个稳的都没有"，
会据此做出**错误的自我定位**（以为只能冲）—— 这不是体验问题，是误导。

### 决策

新增 `recommend_service.allocate_display_slots()`：按**批次配额权重**把 ``limit`` 个展示位
分给各层（水填充 / water-filling）。

1. 用配额权重按最大余额法分配（确定性，复用 `rules.base.distribute_quota`）；
2. 某层候选不足 → 其份额按权重转给**仍有余量**的层，迭代到分完或没有余量；
3. `include_too_risky=True` 时余量才给 `TOO_RISKY`（它不在配额里，默认一位都不给）；
4. 所有层取空仍有余量 → **如实返回少于 limit 条**，绝不用低质量候选凑数；
5. 最终顺序仍是 冲→稳→保→垫（前端按梯度阅读），层内效用降序。

顺带把"批次配额优先、否则回落 `ModelParams.quota`"的权重选取逻辑抽成
`ProvinceRule.quota_weights(batch)`，`default_quota` 改为复用它 ——
原先只有 `default_quota` 里那一份，展示层要用就得复制，复制必然漂移。

### 与 `planner._allocate` 的区别（刻意不同，必须记住）

| | 借位方向 | 理由 |
|---|---|---|
| `planner._allocate`（志愿表） | **向更保守方向借** | 保底是**安全承诺**，不能削弱 |
| `allocate_display_slots`（浏览列表） | 按权重**比例**再分配 | 只要求"各档都能看到"，不代表任何承诺 |

### 新增契约字段（`stats`，ADR-011 强类型）

* `tier_allocation`：本次取样给各层分了多少展示位（只含非零层）；
* `tier_shortfall`：**缺口 = 配额应得 − 候选数**。保底/垫底出现在这里，
  说明这份列表**没有真正的保底**。

同时新增警告（复用既有 `warnings` 通道，前端 `WarningList` 直接显示）：

> ⚠️ 本次取样缺少 DIAN、BAO 档候选（配额要求的数量给不出）：常见原因是可用历史年数不足、
> 安全闸门把「保/垫」降级为「稳」，或筛选条件过窄。**不要据此认为已有保底**。

这同时落实了 M7 清单第 11 项（前端"保底志愿缺失"提示）的**后端部分**；
前端在 `Recommend.tsx` 增加了"各档取样"展示与保底缺失的醒目提示。

### 被否决的方案

| 方案 | 否决理由 |
|---|---|
| 按 `utility` 全局排序取前 N（不分层） | 效用里 `region/level/city` 权重相同，会让"地区好但层次低"的单位挤掉真正的稳档，梯度完全不可读 |
| 只取每层 top-k（k 固定） | 与省份志愿数、配额脱钩；`limit` 变化时行为不连续，且小 limit 下会超出总数 |
| 给 TOO_RISKY 也分一份配额 | 它默认被排除（`include_too_risky=False`），分给它等于凭空占位；只在显式开启时兜余量 |
| 用 `planner._allocate` 复用同一套借位 | 那是"向保守借"的安全语义，会把浏览列表也拉向保底，与"各档都能看到"的目标相反 |
| 前端自行按 tier 分组截断 | 契约铁律：分层取样是**算法决策**，必须在后端算完再下发；前端重算必然与 `tier_distribution` 口径漂移 |

### 验证

* 新增 `tests/test_recommend_truncation.py`（19 例）：纯函数分配语义（比例、空档再分配、
  容量上限、TOO_RISKY 默认不给、缺口语义、确定性、limit=0/负、全空、顺序志愿无配额）
  + 端到端（limit=8/20/60 必须混合档、返回数 ≤ limit、证据链不丢、顺序仍是梯度、
  默认不含过险档）。
* 全量 **494 测试通过**，`core` 覆盖率 **96.34%**。
* 回测（浙江/山东/北京）**数字与改动前完全一致** —— 因为回测走 `evaluate_candidates`，
  不经 `recommend` 的截断；这正好证明本次改动**只影响展示层取样，不影响概率与分层**。
* `frontend/openapi.snapshot.json` 已用文档路径 `node scripts/gen-api-types.mjs --update-snapshot`
  从实时 schema 刷新（21 端点 / 69 schema）；`pnpm typecheck` 通过。


---

## ADR-021 · 会话历史定序：`time.time_ns()` 在 Windows 上不足以做时间序

- **日期**：2026-09
- **状态**：已采纳
- **触发**：M7 验收时全量测试**偶发失败**（约 1/3 概率），
  `test_api.py::test_chat_stream_and_history` 报
  `At index 2 diff: 'assistant' != 'user'`。排查确认**不是测试问题，是产品缺陷**。

### 缺陷（已复现并取到真实数据）

`chat_service._next_id()` 原先是：

```python
return f"msg-{time.time_ns():020d}-{uuid.uuid4().hex[:6]}"
```

docstring 声称"纳秒前缀让 id 本身就是时间序"。但 **Windows 的系统时钟粒度约 15.6ms**，
`time.time_ns()` 在**同一 tick 内返回完全相同的值**，于是同 tick 的两条消息
只能靠**随机后缀**决定先后 —— 50% 概率倒置。实测落库数据：

```
assistant  msg-01790044128378781800-c9aac9   ← 纳秒完全相同
user       msg-01790044128378781800-e1aee5   ← 后缀 e1 > c9 → 助手排到了用户前面
```

会话历史按 `(created_at, id)` 排序，而 `created_at` 只精确到**秒**，
所以同一秒内的先后**完全**由 id 决定。

**影响面（用户可见）**：会话历史是"我当时问了什么、系统依据什么这么答"的**证据链**
（`chat_messages` 的 docstring 明确写了这一点）。顺序错乱意味着证据链读不通 ——
一问一答被排成"答、问"。**不是体验问题，是证据链损坏。**

### 决策

id 增加**进程内单调序号**，保证同一 tick 内也严格递增：
...
* **纳秒前缀**（20 位零填充，字典序 == 数值序）：跨 tick 时天然有序；
* **进程内序号**：同 tick 内纳秒相同时，由它保证递增；
* **随机后缀**：只保证唯一，**不参与**时序判断。

**为什么不用 schema 变更**（如加 `seq INTEGER` 列或改用 `rowid` 排序）：
本机 `create_all` 不给已存在的表加列（HANDOVER 已记），加列必须 `--reset`，
代价是清空考生档案/志愿表/对话；而 id 内嵌序号**零迁移**且语义自洽。
`rowid` 在删除最大行后可能被复用，不是可靠的定序键。

**跨进程并发写同一会话**仍有理论上的定序歧义 —— 那是分布式定序问题，
本系统单 worker 部署，不引入额外机制（已在 docstring 写明）。

### 验证

* 新增 `tests/test_chat_ordering.py`（5 例）：**把时钟冻结在同一 tick**（即原缺陷的触发条件），
  断言 50 个 id 严格递增、形状正确，以及落库后按 `(created_at, id)` 读出的顺序
  等于插入顺序（4 条 / 30 条两种规模）。**确定性**复现，不再依赖运气。
* 真实问答流程跑 **40 轮**（每轮 3 次对话、6 条消息）：修复前约 1/2 倒置，
  修复后 **0 次**。
* 全量测试连跑 4 次**全部通过**（修复前约 1/3 失败）。

### 被否决的方案

| 方案 | 否决理由 |
|---|---|
| 把测试改成"按角色集合断言，不管顺序" | 那等于承认证据链可以乱序；掩盖真实缺陷 |
| 给 `created_at` 加毫秒/微秒精度 | 只把窗口从 15.6ms 缩到 1ms，仍是"赌时钟"；且要改表（`--reset`） |
...
## ADR-023 · 向导意向地区扩展至全国、列表定位与统一导航

- **日期**：2026-09
- **状态**：已采纳
- **背景**：用户反馈意向地区只显示当前六省、推荐结果缺少按院校定位、志愿表梯度分布无法定位到实际志愿，且向导每步重复显示前进/完成按钮。

### 决策

1. 意向地区选择器复用 `REGION_LABEL` 中院校库覆盖的 31 个省级地区；参加高考的省份仍从后端六省规则包选择，不宣称其他地区的报考规则已支持。
2. 推荐页提供大学名搜索，在当前推荐结果中定位匹配院校；同校有多条专业/专业组结果时可用「上一个 / 下一个」逐条定位，并自动展开所在梯度。
3. 志愿表梯度柱及其文字按钮都定位到该档首个志愿，零数量梯度不可点。
4. 建档步骤统一使用页面底部导航操作，移除步骤内容里的重复前进/完成按钮；保存错误在统一操作区展示。

### 验证

* `pnpm typecheck` 通过；`pnpm test` **14 项通过**（含全国地区文案用例）；`pnpm build` 通过（ECharts 分包超过 500 kB 的既有提醒）；`pnpm smoke` 闭环通过；`git diff --check` 通过。
* GPT-6 Sol 对最终工作区只读验收为 **PASS**；审查发现的搜索结果缩小时索引越界、非六省意向显示内部代码、定位后键盘焦点未移动、快速连续搜索触发过期定位回调、保存期间仍可切换步骤五处问题均已修复，最终复核未发现阻断项。
* 前后端本地健康检查通过；桌面浏览器自动化枚举失败（`nodeRepl.fetch request failed`），未完成 GUI 视觉检查。

### 被否决的方案

| 方案 | 否决理由 |
|---|---|
| 让前端列表的 31 个意向地区同时成为可报考省份 | 只有六省规则、科目池与招生数据受支持；两类选择器语义不同，混用会误导考生。 |
| 搜索时只滚动但不展开梯度 | 目标卡片可能位于折叠分组内，滚动无法让卡片可见。 |

## ADR-024 · 局部效用搜索保持志愿梯度分布

- **日期**：2026-09
- **状态**：已采纳
- **背景**：用户反馈更改档案要求/偏好后重新生成志愿表，冲稳保垫梯度可能变得极度不均匀。

### 缺陷与决策

`planner._allocate()` 先按批次配额选择候选，但之后 `_local_search()` 原先允许任何 Tier 的高效用候选替换任何现有志愿。偏好权重只改变效用，不应重写算法已经分配的梯度名额；此前局部搜索可让高效用档位挤占其他档位。

现在局部搜索只在同一 Tier 内交换候选，分层名额严格沿用 `_allocate()` 结果。验收时还发现 `_allocate()` 在更保守层不足后会向更激进层借位，并用任意剩余候选填满空额，与 R-009 冲突；现已收紧为只向更保守层借位，无可借候选时保留空位并显示缺档/总量警告。院校专业组去重仍由 `college_id + group_code` 守护，不能凭空补造缺少的梯度候选。

### 后果与边界

* 偏好效用变化、局部搜索不再改变已分配的梯度数量；有候选供给时配额分布保持稳定。
* 硬筛选若移除某些 Tier 的全部合格候选，原计划总量与冲稳保垫目标可能无法同时满足。系统只从更保守层借位；无保守候选时宁可留空并告知短缺，不放松用户硬约束，也不拿激进候选伪装安全层。
* 局部效用优化只在原有梯度内进行；这以跨梯度效用最优性换取梯度结构稳定，符合配额作为生成规则的语义。

### 验证

* `backend/tests/test_planner.py` 定向测试 12 项通过；直接调用 `_local_search` 覆盖同 Tier 替换、跨 Tier 诱饵保持梯度，以及重复院校专业组拒绝换入；分配测试验证更激进候选不补保守档空缺。
* 全量后端测试及浙江回测已在新增严格借位规则后复跑，结果记录如下。
* 全量后端测试：517 项通过，core 覆盖率 96.52%（门槛 90%）。
* 浙江 2025 回测命令通过；稳档命中率 95.11%、冲档命中率 38.82%、Brier 0.0510、安全网失效率 0%。该回测样本无 BAO/DIAN 志愿，不能证明保底有效。回测衡量推荐概率与历史数据，不执行志愿表分配。
* GPT-6 Sol 两轮只读验收：PASS；确认定向测试覆盖同 Tier 替换、跨 Tier 分布保持、重复院校专业组拦截及更激进候选不补保守档空缺，并确认 `_allocate` 仅向更保守层借位、缺额留空告警。

### 被否决的方案

| 方案 | 否决理由 |
|---|---|
| 局部搜索继续跨 Tier 换入高效用候选 | 选择偏好权重时会间接改变梯度配额，生成结果的安全结构随 UI 偏好漂移。 |
| 为缺失梯度放宽用户的硬筛选或把其他候选重标为该梯度 | 会违反用户约束或改变概率算法结果，不能用来制造“看起来均衡”的志愿表。 |
