...
_INTENT_RULES: tuple[tuple[str, re.Pattern[str]], ...] = (
    ...
    # ★ ADR-019：院校**层次/实力**类问题。
    #   必须排在 `history` **之后**（"XX大学多少分"是 history，不是层次），
    #   排在 `rank` **之前**（"XX大学什么水平"里没有"位次/排名"，但"XX大学排名第几"含"排名"，
    #   那种情况交给 rank 也合理）。
    #   为什么必须有这条：原先这类问题落到 `unknown`，被 `_answer_about_school` 当成
    #   "问分数线"回答 —— 考生问"浙工大怎么样"，得到一堆位次数字，答非所问（实测踩到）。
    (
        "college_level",
        re.compile(
            r"怎么样|好不好|咋样|什么水平|啥水平|水平如何|算不算好|算好学校吗|实力如何|实力怎么样"
            r"|强不强|厉害吗|值得报吗|值得上吗|是不是好学校|层次|什么档次|什么级别"
        ),
    ),
