 ...
 * 缺口提示由文本与风险面板承担。
 */
export function GradientChart({ distribution, maxVolunteers, height = 200 }: GradientChartProps) {
  const counts = useMemo(() => distribution ?? {}, [distribution])
  const option = useMemo<EChartsCoreOption>(() => {
    const tiers = TIER_ORDER.filter((tier) => tier !== 'NO_DATA')
    return {
      grid: { left: 8, right: 16, top: 24, bottom: 4, containLabel: true },
      tooltip: { trigger: 'axis', axisPointer: { type: 'shadow' } },
      xAxis: {
        type: 'category',
        data: tiers.map((tier) => TIER_STYLE[tier].name),
        axisTick: { show: false },
      ...
      series: [
        {
          ...
          data: tiers.map((tier) => ({
            value: counts[tier] ?? 0,
            itemStyle: { color: TIER_STYLE[tier].hex, borderRadius: [4, 4, 0, 0] },
          })),
  ...
  return (
    <div>
      <Chart option={option} height={height} ariaLabel="志愿表冲稳保垫分层分布柱状图" />
