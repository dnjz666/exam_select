...
2. ★ **性能**：`evaluate_candidates` 在浙江真实数据下 **5.9–8.4s**（模拟侧只要 0.6s），
   远未达到 P95 < 1s。瓶颈是"18,543 个单位逐个算概率"。
   本轮已做等价优化（25s → 5.9s，见 ADR-015）；下一步需要**架构改动**：
   先按粗排（层次/位次档）筛出候选再精算，或按桶预计算。改之前先读 ADR-015 的性能小节。
