...
| **M7** | **加固与交付（进行中：分层截断 ✅ / 会话定序 ✅ / 志愿局部搜索保梯度 ✅）** | 🟡 **进行中** |
| M7 剩余 | 回测口径 · 性能 P95<1s · 安全边界 · 视觉回归 · Docker · 手册/alembic 等 | ⬜ 待做 |
...
* ✅ **志愿表保持梯度安全边界**（ADR-024）：效用搜索只在同一 Tier 替换；借位只从更保守层补入，
  无合格保守候选时保留空位并提示，不用更激进候选伪装保底。定向 planner 测试 12 项通过；
  全量后端测试 517 项通过（core 覆盖率 96.52%），GPT-6 Sol 复验 PASS。
...
3. ✅ **已完成（ADR-020）· 推荐列表截断方式**：原实现"按 `(tier, -utility)` 排序取前 N"
   ...
   **注意**：展示层取样与志愿表分配用途不同，别把两者合并；志愿表借位按 R-009 执行，见 ADR-024。
...
5. 边界：0 志愿 / 超高分 / 超低分 / 全被过滤 / 一次对话内连续建档。分配阶段借位现已按
   R-009 收紧（ADR-024）；以上端到端边界场景仍需补齐。
...
12. 🟡 **"保底志愿缺失"提示 —— 逻辑与构建已完成，前端待视觉验证**：
    后端现在通过 `stats.tier_shortfall` + `warnings` 明确披露（ADR-020）；
    `Recommend.tsx` 已加醒目提示；`pnpm typecheck`、`pnpm test`、`pnpm build` 和 `pnpm smoke`
    均已通过（2026-09，见 ADR-022）。桌面浏览器自动化枚举失败，仍需 Playwright/CUA 视觉回归。
    ⚠️ 另注意：**推荐列表里 BAO/DIAN 是有候选的**（全池 BAO 62 / DIAN 7824），
    缺的是**志愿表**侧（2 年窗口 + 安全闸门把保/垫降级为稳，见 ADR-015 缺陷 6）。
13. ✅ **首次回复免责声明来源口径已修正**：不再硬编码"当前数据为模拟数据"；免责声明
    改为按每项结果标注数据性质，见当前 `chat_service.DISCLAIMER` 实现。
