...
```
冲 37.5%  |  稳 37.5%  |  保 18.75%  |  垫 6.25%
```
即 **冲+稳 = 75%（且冲 ≈ 稳）、保+垫 = 25%**（2026-09 / ADR-019 由 `25/40/25/10` 调整）。
浙江 80 个志愿的整数结果：**冲 30 / 稳 30 / 保 15 / 垫 5**。
**结构校验**：垫底志愿数量 ≥ 3 且 ≥ 5% 总志愿数；若考生总志愿数 < 10，垫底 ≥ 2。
> 垫底下限**优先于配额**：总志愿数较小时（如上海 24 个），配额算出的垫底可能低于下限，
> `planner._allocate` 会从更保守方向调入，不会被配额突破（名师铁律 4）。
...
2. 若某层候选不足，只能向更保守的层借位；没有更保守候选时保留空位并明确警告，严禁用更激进候选填充保守档
3. **全局排序**：投影为「考生意愿序」，但强制满足：最后一档必须是 DIAN/BAO
4. **局部搜索优化**：尝试 swap 提升总效用，但只允许在**同一 Tier 内**替换；分层配额完成后的数量是硬约束，改变偏好权重不得改变梯度分布。候选层供给不足导致借位或留空时，如实保留缺档与总量警告。
